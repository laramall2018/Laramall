 ...
 *  MYSQL 公用类库
 * ============================================================================
 * * 版权所有 2005-2012 北京麦维创想科技有限公司，并保留所有权利。
 * 网站地址: http://www.mywebphp.com；
 * ----------------------------------------------------------------------------
 * 这不是一个自由软件！您只能在不用于商业目的的前提下对程序代码进行修改和
 * 使用；不允许对程序代码以任何形式任何目的的再发布。
 * ============================================================================
 * $Author: sunweihua $
 * $Id: Mwmysql.php 17217 2011-01-19 06:29:08Z swh $
*/
...
class Mwdb
{
    ...
    function cls_mysql($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        ...
        $charset = strtolower(str_replace('-', '', 'utf8'));
    ...
    {   
            ...
            $this->cls_mysql($db_host, $db_user, $db_pass, $db_name, 'utf8', $pconnect, $quiet);
    }

    function table($table){

        return Config::get('database.connections.mysql.prefix').$table;
    }
    ...
    function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0)
    {
        if ($pconnect)
        {
            if (!($this->link_id = mysql_pconnect($dbhost, $dbuser, $dbpw,$dbname)))
            {
                if (!$quiet)
                {
