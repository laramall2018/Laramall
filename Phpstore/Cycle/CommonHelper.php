...
class CommonHelper{
    ...
    */
    public function tableDataInit(){
        ...
        $tableData->addField('is_delete',1);
    ...
    */
    public function getData($data){
        ...
        foreach($data as $key=>$value){
            ...
            //alias赋值
            $data[$key]['goods_thumb_str']          = Common::get_goods_thumb($value['id']);
            ...
            $data[$key]['brand_name']               = Common::get_brand_name($value['brand_id']);
            $data[$key]['cat_name']                 = Common::get_cat_name($value['cat_id']);

            //操作链接
            $data[$key]['edit_url']                 = Common::get_resource_edit_url('admin/cycle',$value['id']);
    ...
    */
		...
		*/
		public function goods($id){
				...
				DB::table('goods_relation')->where('goods_id',$id)->delete();
				DB::table('goods_relation')->where('relation_goods_id',$id)->delete();
				//插入关联商品编号信息
				foreach($ids as $item){

					  $data 	= ['goods_id'=>$id, 'relation_goods_id'=>$item ];

						if($id != $item){

							 DB::table('goods_relation')->insert($data);
						}
		...
		*/
		public function article($id){
			 ...
			 DB::table('goods_article')->where('goods_id',$id)->delete();

			 foreach($article_ids as $article_id){

				   DB::table('goods_article')->insert(['goods_id'=>$id , 'article_id'=>$article_id]);
			 }
