...
use Phpstore\OrderReturn\CommonHelper;
use Request;
use Validator;
...
class ReturnController extends BaseController{
    ...
    */
    public function index(){
        ...
        $view->page                 = $this->page;
        $view->tag                  = $this->tag;
        $current_url                = HTML::link($this->list_url,trans('admin.order_return_list'));
        $view->path_url             = $this->get_path_url($current_url);
        $view->action_name          = trans('admin.order_return_list');

        //生成添加按钮
        $view->add_btn              = Common::get_add_btn($this->add_url,trans('admin.add_order_return'));
        ...
        //$grid->portlet()
        $grid->put('color','blue');
        $grid->put('action_name',trans('admin.order_return_list'));
        $view->grid                 = $grid;
    ...
    */
    public function create(){
        ...
        $view->page         = $this->page;
        $view->tag          = $this->tag;
        $current_url        = HTML::link($this->add_url,trans('admin.add_order_return'));
        $view->path_url     = $this->get_path_url($current_url);

        $view->action_name  = trans('admin.add_order_return');
    ...
    public function store(){
         ...
         if($this->form_to_model->insert()){
              ...
              if($order){
                 ...
                 $this->help->return_status_sync($model->id , $order->id);
              }
              ...
              $order_log->log           = '添加了退货单: 订单单号：'.$model->order_sn; 
              $order_log->log();
    ...
    */
    public function edit($id){
        ...
        $view                       = $this->view('crud_add');
        $view->action_name          = trans('admin.edit_order_return');
        $view->page                 = $this->page;
        $view->tag                  = $this->tag;
        $current_url                = HTML::link($this->edit_url.$id,trans('admin.edit_order_return'));
        $view->path_url             = $this->get_path_url($current_url);
    ...
    */
    public function update(){
        ...
        if($this->form_to_model->insert()){
              ...
              $order_log->username      = Auth::user('admin')->username;
              $order_log->order_sn      = $model->order_sn;
              $order_log->log           = '修改订单退货信息,订单号：'.$model->order_sn; 
              $order_log->log(); 
