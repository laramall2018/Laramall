...
use App\Models\CatAd;
use File;
use HTML;
use Input;
use Phpstore\Base\Sysinfo;
use Phpstore\CatAd\CommonHelper;
...
|---------------------------------------------------------------------------------------
*/
class CatAdController extends BaseController{
    ...
    */   
    ...
    function __construct(){
    	...
    	parent::__construct();
        $this->page                 = 'config';
        $this->tag                  = 'admin.catad.index';
        $this->crud                 = new Crud();
        $this->form_to_model        = new FormToModel();

        //定义商品的常用操作链接
        $this->list_url             = 'admin/catad';
        $this->edit_url             = 'admin/catad/edit';
        $this->add_url              = 'admin/catad/create';
        $this->update_url           = 'admin/catad/update';
        $this->del_url              = 'admin/catad/del/';
        $this->batch_url            = 'admin/catad/batch';
        $this->preview_url          = '';
        $this->ajax_url             = 'admin/catad/grid';
    ...
    */
    public function index(){
        ...
        $view->page                 = $this->page;
        $view->tag                  = $this->tag;
        $current_url                = HTML::link($this->list_url,trans('admin.catad_list'));
        $view->path_url             = $this->get_path_url($current_url);
        $view->action_name          = trans('admin.catad_list');

        //生成添加按钮
        $view->add_btn              = Common::get_add_btn($this->add_url,trans('admin.add_catad'));
        ...
        //$grid->portlet()
        $grid->put('color','blue');
        $grid->put('action_name',trans('admin.catad_list'));
        $view->grid                 = $grid;
    ...
    */
    public function create(){
        ...
        $view->page         = $this->page;
        $view->tag          = 'admin.catad.index';
        $current_url        = HTML::link($this->add_url,trans('admin.add_catad'));
        $view->path_url     = $this->get_path_url($current_url);

        $view->action_name  = trans('admin.add_catad');
    ...
    */
    public function edit($id){
        ...
        $view                       = $this->view('crud_add');
        $view->action_name          = trans('admin.edit_catad');
        $view->page                 = $this->page;
        $view->tag                  = $this->tag;
        $current_url                = HTML::link($this->edit_url.$id,trans('admin.edit_catad'));
        $view->path_url             = $this->get_path_url($current_url);
